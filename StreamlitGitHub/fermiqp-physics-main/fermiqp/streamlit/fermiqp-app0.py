...
from fermiqp.loaddata import load_zeeman, load_scatteringlengths, load_lattice1d, load_lattice2d
...
def main():
    ...
    tab0, tab1, tab2,tab3,tab4,tab5 = st.tabs(["Fermi Gas", "Sheet Trap", 
                          "Pinning Lattice",
                          "Superlattice",
                          "Fields",
                          "Lasers"
...
### Sheet trap
    ...
    with tab1:
        ...
        capture_output(lambda: ol.DipoleTrap(waist_horz*1e-6,
                                            waist_vert*1e-6,
                                            U=ol.power2freq(power,
                                                            waist=waist_horz*1e-6,
                                                            waisty=waist_vert*1e-6)))
        output_text.text("\n".join(captured_output))  # Display the captured output

        sheet = ol.DipoleTrap(waist_horz*1e-6,
                            waist_vert*1e-6,
                            U=ol.power2freq(power,
                                            waist=waist_horz*1e-6,
                                            waisty=waist_vert*1e-6))
    ...
    #     output_text = st.empty()
        ...
        capture_output(lambda: ol.OpticalLattice1D(a=a*1e-9,
                                                    U=eta_boost*ol.power2freq(power))) 
        output_text.text("\n".join(captured_output))  # Display the captured output

        waist_horz=waist_horz*1e-6
        waist_vert=waist_vert*1e-6
        if config == "2 x 1D lattice":
            latt =  ol.OpticalLattice1D(a=a*1e-9,
                                U=eta_boost*ol.power2freq(power, waist=waist_horz, waisty=waist_vert)/2)
        else:
            latt =  ol.OpticalLattice1D(a=a*1e-9,
                                U=eta_boost*ol.power2freq(power, waist=waist_horz, waisty=waist_vert))
        trap =  ol.DipoleTrap(U=eta_boost*ol.power2freq(power, waist=waist_horz, waisty=waist_vert),
                        waist=waist_horz,
                        waisty=waist_vert)

        if config != "Dipole trap":
            st.write('')
            st.write(f"Lattice spacing: {a:d} nm; Lattice recoil: {latt.Er*1e-3:2g} kHz; Harmonic length: {np.sqrt(ol.hbar/ol.mass/latt.trap_freq()/2/np.pi)*1e9:.2f} nm")
    ...
    with tab3:
        ...
        capture_output(lambda: ol.OpticalLattice1D(a=a*1e-9,
                                                    U=eta_boost*ol.power2freq(power,                        waist=waist_horz*1e-6,
                        waisty=waist_vert*1e-6, wavelen=wavelen))) 
        output_text.text("\n".join(captured_output))  # Display the captured output
        
        latt =  ol.OpticalLattice1D(a=a*1e-9,
                                U=eta_boost*ol.power2freq(power,                        waist=waist_horz*1e-6,
                        waisty=waist_vert*1e-6, wavelen=wavelen))
        trap =  ol.DipoleTrap(U=eta_boost*ol.power2freq(power,                         waist=waist_horz*1e-6,
                        waisty=waist_vert*1e-6, wavelen=wavelen),
                        waist=waist_horz*1e-6,
                        waisty=waist_vert*1e-6)

        st.write(f"Lattice spacing: {int(a):d} nm; Lattice recoil: {latt.Er*1e-3:2g} kHz; Harmonic length: {np.sqrt(ol.hbar/ol.mass/latt.trap_freq()/2/np.pi)*1e9:.2f} nm")
    ...
    with tab4:
        ...
        with cols[0]:
            ...
            for i in range(3):
                ...
                ax.plot(B, E, '-', label=fr'${i+1}- |-3/2, {1-i}\rangle$: {E[iB0]*1000:.0f} MHz')
        ...
        with cols[1]:
            ...
            for i in range(3):
                ...
                ax.plot(B, E, '-', label=fr'${i+4}- |3/2, {1-i}\rangle$: {E[iB0]*1000:.0f} MHz')
    ...
    with tab5:
        ...
        st.write(fr"$s_0$ = {s0:g}; $\Omega/2\pi$ = {Omega:g} MHz")
        ...
        # ax.plot(x, 
        im = ax.contourf(X, Y, v, levels=[0.025, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5], cmap='diverging')
